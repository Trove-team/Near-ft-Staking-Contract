...
export default function Project() {
  ...
  useEffect(() => {
    ...
    async function fetchProjectDeatils(id) {
      const res = await fetch(
        `https://jump-pg-prisma-api-production.up.railway.app/api/telos-projects/${id}`
      );
  ...
  useEffect(() => {
    const url =
      "https://jump-pg-prisma-api-production.up.railway.app/api/fetch/telos/" +
      id;
  ...
  async function handleWhitelist() {
    ...
    if (!addToWhiteListMany) {
      ...
      const provider = new JsonRpcProvider("https://mainnet.telos.net/evm");
          ...
          <div className="bg-[rgba(255,255,255,0.1)] p-[24px] rounded-[20px] w-full relative details">
            ...
            {tab === "pool" && parsedProject && (
              <MantleProjectStats
                ...
                vestingTokenDecimals={vestingTokenDecimals}
              />
            )}
