...
export default function MantleVesting() {
  ...
  async function handleClaimLockedJump() {
    //https://mantletestnetfacuetbackend-production.up.railway.app/api/reward-locked-jump
    ...
    try {
      const res = await toast.promise(
        ...
        {
          loading: "Claiming locked jump...",
          success: null,
          error: "Error while claiming locked jump",
        }
      );

      if (res.status !== 200)
        throw new Error("Error while claiming locked jump");
      const data = await res.json();
      toast.success("Claimed locked jump");
      window.location.reload();
    } catch (e) {
      toast.error("Error while claiming locked jump");
    }
  }
