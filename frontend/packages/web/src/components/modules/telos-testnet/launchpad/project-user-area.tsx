...
export default function MantleUserArea({
  ...
  const { data: saleTokenDecimals } = useContractRead({
  ...
  const { data: vestingTokenDecimals } = useContractRead({
  ...
  async function handleInvest() {
    if (!approved) {
      if (!approveWrite) {
        ...
        try {
          await fetch(
            "https://jump-pg-prisma-api-production.up.railway.app/api/telos-projects/" +
              projectData.id
          );
        } catch (e) {
          //
  ...
  const currentPrice = useCallback(() => {
    switch (status()) {
      ...
      case MantleStatusEnum.Private:
        return ethers
          .formatUnits(projectData.privateSalePrice, saleTokenDecimals || 18)
          .toString()
          .replace(/\.?0+$/, "");
      case MantleStatusEnum.Public:
        return ethers
          .formatUnits(projectData.publicSalePrice, saleTokenDecimals || 18)
          .toString()
          .replace(/\.?0+$/, "");
      case MantleStatusEnum.Closed:
        return "0";
    }
  }, [projectData, status, saleTokenDecimals]);

  const remainingAllocation = useCallback(() => {
    const total = parseInt(
      formatUnits(projectData.totalSaleAmount as any, saleTokenDecimals || 18)
    );
    const sold = parseInt(
      formatUnits(projectData.totalSoldAmount as any, saleTokenDecimals || 18)
    );
  ...
  return (
    <div className="investment relative bg-[rgba(255,255,255,0.1)] rounded-[20px] py-[24px] pb-[64px] px-[64px] flex-1 flex flex-col items-center h-max mb-[8px] max-w-[548px]">
      ...
      <Button
        className="w-full"
        disabled={amount == 0 || !address || disabled}
      ...
      <p className=" mt-6 text-gray-300 text-sm mb-3">
        {saleTokenSymbol} Balance :{" "}
        {ethers.formatUnits(balance, saleTokenDecimals || 18)}
      </p>
    </div>
  );
}
